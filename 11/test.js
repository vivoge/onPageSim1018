...
}//形参传入的是document，三元表达式选择all 或者用通配符选择所有元素
...
//    }
    ...
    for (var i = 0; i < tokens.length; i++) {
        var token = tokens[i].replace(/^\s+/,'').replace(/\s+$/,'');  //div.fileDiv
...
}//鼠标xy坐标返回一个对象{x:posX, y:posY}

var _selectedRegions = [];//这个是个选中DIV的数组，里面存放被选中的div


function RegionSelect(selRegionProp){
    this.regions =[];
    var _regions = document.getElementsBySelector(selRegionProp["region"]);//返回一个选择好DOM的数组
    if(_regions  &&  _regions.length > 0){
        var _self = this;                       //保存一个this


        for(var i=0; i< _regions.length;i++){
            _regions[i].onclick = function(){
    ...
    this.selectedClass = selRegionProp["selectedClass"];//seled
    this.selectedRegion = [];
    this.selectDiv = null;
    this.startX = null;
    this.startY = null;
...
RegionSelect.prototype.onBeforeSelect = function(evt){
    if(!document.getElementById("selContainer")){
        ...
        this.selectDiv.style.cssText = "position:absolute;width:0px;height:0px;font-size:0px;margin:0px;padding:0px;border:1px solid #0099FF;background-color:#C3D5ED;z-index:1000;filter:alpha(opacity:60);opacity:0.6;display:none;";
...
};//创建了一个选区div在html最下面，并且初始化了startX和startY的值，isselect为true

RegionSelect.prototype.onSelect = function(evt){
    var _self = this;
    if(_self.isSelect){
        if(_self.selectDiv.style.display == "none") _self.selectDiv.style.display = "block";

        var posX = posXY(evt).x;
        var poxY = posXY(evt).y;

        _self.selectDiv.style.left   = Math.min(posX, this.startX)+'px';
        _self.selectDiv.style.top    = Math.min(poxY, this.startY)+'px';
